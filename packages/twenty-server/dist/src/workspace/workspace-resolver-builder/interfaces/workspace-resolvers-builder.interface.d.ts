...
import { Record, RecordFilter, RecordOrderBy } from 'src/workspace/workspace-query-builder/interfaces/record.interface';
...
export interface FindManyResolverArgs<Filter extends RecordFilter = RecordFilter, OrderBy extends RecordOrderBy = RecordOrderBy> {
    first?: number;
    last?: number;
    before?: string;
    after?: string;
...
export interface UpdateManyResolverArgs<Data extends Record = Record, Filter = any> {
    filter: Filter;
    data: Data;
}
...
export type WorkspaceResolverBuilderQueryMethodNames = (typeof workspaceResolverBuilderMethodNames.queries)[number];
export type WorkspaceResolverBuilderMutationMethodNames = (typeof workspaceResolverBuilderMethodNames.mutations)[number];
export type WorkspaceResolverBuilderMethodNames = WorkspaceResolverBuilderQueryMethodNames | WorkspaceResolverBuilderMutationMethodNames;
